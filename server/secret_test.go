...
func SecretCRUDForSSHTemplate(t *testing.T, tss *Server) {
	...
	sc.SshKeyArgs = nil
	// Note: We don't try to change the filename because the server preserves
	// the template-defined field name ("Public Key" in this case)
	...
	if publicKeyField, problem := getField(sc, publicKeyFieldId, t); publicKeyField != nil && !problem {
		...
		// Verify the filename is preserved from template (should be the field name)
		if !validate("updated secret public key field has the template field name", publicKeyField.FieldName, publicKeyField.Filename, t) {
			return
		}
	} else if problem {
		return
